...
public class CacheControllerTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(httpExecutionContext.current()).
    ...
    public void addObject() throws Exception {
        Mockito.when(requestBody.asJson()).
                thenReturn(null);
        ...
        Mockito.when(requestBody.asJson()).
                thenReturn(cacheEntryJsonNode);
        Mockito.when(cacheService.addObject("mymap", "c1", 1)).
                thenReturn(CompletableFuture.completedFuture(true));
        assertTrue(contentAsString(cacheController.addObject("mymap").toCompletableFuture().get()).contains("true"));

        Mockito.when(requestBody.asJson()).
                thenReturn(cacheEntryJsonNodeWithTTL);
        Mockito.when(cacheService.addObjectWithExpiry("mymap", "c2", 1, 60)).
                thenReturn(CompletableFuture.completedFuture(true));
        assertTrue(contentAsString(cacheController.addObject("mymap").toCompletableFuture().get()).contains("true"));
    }
    ...
    public void getObject() throws Exception {
        Mockito.when(cacheService.getObject("mymap", "a")).
    ...
    public void deleteMapKey() throws Exception {
        Mockito.when(cacheService.deleteMapKey("mymap", "a")).
                thenReturn(CompletableFuture.completedFuture(true));
    ...
    public void addKeyToSet() throws Exception {
        ...
        Mockito.when(requestBody.asJson()).
                thenReturn(setEntryNode);
        Mockito.when(cacheService.addKeyToSet("mymap", "s1")).
                thenReturn(CompletableFuture.completedFuture(true));
    ...
    public void doesKeyExistInSet() throws Exception {
        Mockito.when(cacheService.doesKeyExistInSet("mymap", "s1")).
